...
let playing = false;
let songs = [];
let canciones_disponibles = document.querySelectorAll(".cancion, .cancion_resultado, .cancion_alternativa, .cancion_playlist, .cancion_escuchada");

/* Para rellenar el json con las canciones que aparecen en la pantalla y guardar en una lista */
canciones_disponibles.forEach(cancion => {
    let song_json = {
        "info_titulo_cancion": "",
    ...
    if (cancion.style.display == "block") {
...
/* Para inicializar el reproductor para que muestre la primera canción*/
...
siguiente.addEventListener("click", function() {
    ...
    console.log("track_actual"+track_actual);
});
...
function updateInfo() {
    ...
    info_artista_cancion.textContent = song.info_artista_cancion;
    caratula_cancion.src = song.caratula_cancion;
    caratula_cancion.addEventListener("load", function() {
        audio.play();
    });
}
